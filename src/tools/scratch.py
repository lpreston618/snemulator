# a = """61 2   7-m+w       (dir,X)   mm....mm . ADC ($10,X)
...
# opcode = lambda l: l.split()
# instr_name = lambda l: l.split()[6].lower()
...
#     opcode = split[0]
#     instr_name = split[6].lower()
#     instr_bytes = split[1]
#     ninstr_bytes = int(instr_bytes[0]) - 1
#     addr_mode = "None"
    
#     if len(split) == 8:
#         addr_mode = split[7]

#     instr_case =  f"(0x{opcode}, [CASE_REST]) => " + "{\n"
#     instr_case += f"    instr_name = \"{instr_name}\".to_string();\n"
#     instr_case += f"    instr_data = [INSTR_DATA_FMT];\n"
#     instr_case += f"    addr_mode_fmt = [ADDR_MODE_FMT];\n"
#     instr_case += "}\n"

#     if "-m" in instr_bytes:
#         case_m8 = instr_case
#         case_m8 = case_m8.replace("[CASE_REST]", "_, true, _")
#         case_m8 = case_m8.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         case_m8 = case_m8.replace("[ADDR_MODE_FMT]", 'format!("#${b0:02X}").to_string()')

#         case_m16 = instr_case
#         case_m16 = case_m16.replace("[CASE_REST]", "_, false, _")
#         case_m16 = case_m16.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         case_m16 = case_m16.replace("[ADDR_MODE_FMT]", 'format!("#${b1:02X}{b0:02X}").to_string()')

#         instr_case = case_m8 + case_m16

#     elif "-x" in instr_bytes:
#         case_x8 = instr_case
#         case_x8 = case_x8.replace("[CASE_REST]", "_, _, true")
#         case_x8 = case_x8.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         case_x8 = case_x8.replace("[ADDR_MODE_FMT]", 'format!("#${b0:02X}").to_string()')

#         case_x16 = instr_case
#         case_x16 = case_x16.replace("[CASE_REST]", "_, _, false")
#         case_x16 = case_x16.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         case_x16 = case_x16.replace("[ADDR_MODE_FMT]", 'format!("#${b1:02X}{b0:02X}").to_string()')

#         instr_case = case_x8 + case_x16
    
#     elif addr_mode == "$10,X":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b0:02X},X").to_string()')
...
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("(${b0:02X},Y)").to_string()')
...
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b0:02X},S").to_string()')

#     elif addr_mode == "#$12":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("#${b0:02X}").to_string()')
...
#     elif addr_mode == "#$12,#$34":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("#${b1:02X},#${b0:02X}").to_string()')
...
#     elif addr_mode == "($1234),X":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("(${b1:02X}{b0:02X}),X").to_string()')
...
#     elif addr_mode == "$123456":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X} {b2:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b2:02X}{b1:02X}{b0:02X}").to_string()')
    
#     elif addr_mode == "$9876,X":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b1:02X}{b0:02X},X").to_string()')
    
#     elif addr_mode == "$9876,Y":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b1:02X}{b0:02X},Y").to_string()')

#     elif addr_mode == "$FEDBCA":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X} {b2:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b2:02X}{b1:02X}{b0:02X}").to_string()')
    
#     elif addr_mode == "$FEDCBA":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X} {b2:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b2:02X}{b1:02X}{b0:02X}").to_string()')

#     elif addr_mode == "$FEDCBA,X":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X} {b2:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b2:02X}{b1:02X}{b0:02X},X").to_string()')

#     elif addr_mode == "($32,S),Y":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("(${b0:02X},S),Y").to_string()')

#     elif addr_mode == "LABEL":
#         if split[3] == "rel8":
#             instr_case = instr_case.replace("[CASE_REST]", f"..")
#             instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X}").to_string()')
#             instr_case = instr_case.replace("[ADDR_MODE_FMT]", '"[REL8]".to_string()')
#         elif split[3] == "rel16":
#             instr_case = instr_case.replace("[CASE_REST]", f"..")
#             instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#             instr_case = instr_case.replace("[ADDR_MODE_FMT]", '"[REL16]".to_string()')
#         elif split[3] == "imm":
#             instr_case = instr_case.replace("[CASE_REST]", f"..")
#             instr_case = instr_case.replace("[INSTR_DATA_FMT]", 'format!("{b0:02X} {b1:02X}").to_string()')
#             instr_case = instr_case.replace("[ADDR_MODE_FMT]", 'format!("${b1:02X}{b0:02X}").to_string()')
#         else:
#             print("Unknown addr mode for op", opcode)

#     elif addr_mode == "None":
#         instr_case = instr_case.replace("[CASE_REST]", "..")
#         instr_case = instr_case.replace("[INSTR_DATA_FMT]", '"".to_string()')
#         instr_case = instr_case.replace("[ADDR_MODE_FMT]", '"".to_string()')
#     # else:
#         # print(f"Unrecognized addr mode '{addr_mode}'")
#     result += instr_case

#         # break

# with open("src/system/scpu/disassembler_match.txt", "w") as f:
#     f.write(result)

# print("Done.")

b = None
with open("games/Super Mario World (USA).sfc", "rb") as f:
    b = f.read()
...
with open("games/Super Mario World (USA) copy.sfc", "wb") as f:
    f.write(b)
