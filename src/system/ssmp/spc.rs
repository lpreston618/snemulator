...
const FAST_TIMER_CLOCK_PERIOD: usize = 16;
const SLOW_TIMER_CLOCK_PERIOD: usize = 128;
...
impl Spc700 {
    ...
    fn write_sound_regs(&mut self, address: u16, data: u8) {
        match address & 0xF {
            0x1 => {
                self.timer0.set_enable(data.bit_en(0));
                self.timer1.set_enable(data.bit_en(1));
                self.timer2.set_enable(data.bit_en(2));
            ...
            0xD => { self.timer0.set_counter(data); }
            0xE => { self.timer1.set_counter(data); }
            0xF => { self.timer2.set_counter(data); }
            _ => {}
        }
    }
