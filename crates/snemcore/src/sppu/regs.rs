...
impl PpuRegs {
    ...
    pub fn write_2105(&mut self, value: u8) {
        ...
        log::debug!("Set bg mode to: {:?}", self.bg_mode);
    }
    ...
    pub fn write_210B(&mut self, value: u8) {
        self.bg_settings[0].chr_base_addr = (((value as u16) >> 0) << 12) & 0x7FFF;
        self.bg_settings[1].chr_base_addr = (((value as u16) >> 4) << 12) & 0x7FFF;
    }
