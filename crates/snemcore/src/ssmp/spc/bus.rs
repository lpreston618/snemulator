...
use crate::ssmp::spc::{ioregs::SpcIoRegs, Spc700};
...
impl<'a, H: DebugHarness> SpcBus<'a, H> {
    pub fn read(&mut self, addr: u16) -> u8 {
        match addr {
            ...
            0xFFC0..=0xFFFF => match self.spc_regs.ipl_read_en {
    ...
    fn write_spcio_regs(&mut self, addr: u8, value: u8) {
        match addr & 0xF {
            ...
            0x1 => {
                // TODO: At reset this register is initialized as if $80 was written to it.
            ...
            0x5 => {
                self.apuio_regs.apuio1 = value;
            }
            0x6 => {
                self.apuio_regs.apuio2 = value;
            }
            0x7 => {
                self.apuio_regs.apuio3 = value;
            }
            0xA => {
                self.timer0.set_target(value);
            }
            0xB => {
                self.timer1.set_target(value);
            }
            0xC => {
                self.timer2.set_target(value);
            }
            0xD => {} // Read-only reg
            0xE => {} // Read-only reg
            0xF => {} // Read-only reg
    ...
    fn read_sdsp_regs(&mut self) -> u8 {
        ...
        match nibble_lo {
            ...
            0xC => {
                match nibble_hi {
                    ...
                    7 => {
                        ...
                        endx |= if self.voice_regs[7].end_of_sample_flag {
                            0x80
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[6].end_of_sample_flag {
                            0x40
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[5].end_of_sample_flag {
                            0x20
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[4].end_of_sample_flag {
                            0x10
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[3].end_of_sample_flag {
                            0x08
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[2].end_of_sample_flag {
                            0x04
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[1].end_of_sample_flag {
                            0x02
                        } else {
                            0
                        };
                        endx |= if self.voice_regs[0].end_of_sample_flag {
                            0x01
                        } else {
                            0
                        };
            ...
            0xD => {
                match nibble_hi {
                    ...
                    2 => {
                        ...
                        pitchmod_en |= if self.voice_regs[7].pitchmod_en {
                            0x80
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[6].pitchmod_en {
                            0x40
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[5].pitchmod_en {
                            0x20
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[4].pitchmod_en {
                            0x10
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[3].pitchmod_en {
                            0x08
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[2].pitchmod_en {
                            0x04
                        } else {
                            0
                        };
                        pitchmod_en |= if self.voice_regs[1].pitchmod_en {
                            0x02
                        } else {
                            0
                        };
            ...
            0xF => self.sdsp_regs.fir_regs[(nibble_hi & 0x7) as usize] as u8,
    ...
    fn write_sdsp_regs(&mut self, value: u8) {
        ...
        match nibble_lo {
            0x0..=0xB => {
                ...
                match nibble_lo {
                    0 => {
                        voice.lchannel_volume = value;
                    }
                    1 => {
                        voice.rchannel_volume = value;
                    }
                    2 => {
                        set_byte_n!(voice.pitch, value as u16, 0);
                    }
                    3 => {
                        set_byte_n!(voice.pitch, (value & 0x3F) as u16, 1);
                    }
                    4 => {
                        voice.sample_source = value;
                    }
                    5 => {
            ...
            0xC => {
                match nibble_hi {
                    0 => {
                        self.sdsp_regs.lmain_volume = value;
                    }
                    1 => {
                        self.sdsp_regs.rmain_volume = value;
                    }
                    2 => {
                        self.sdsp_regs.lecho_volume = value;
                    }
                    3 => {
                        self.sdsp_regs.recho_volume = value;
                    }
                    4 => {
                        ...
                        for voice_idx in 0..8 {
                            ...
                            if get_bit_n!(value, voice_idx)
                                && !get_bit_n!(self.sdsp_regs.key_off, voice_idx)
                            {
                                voice.adsr_stage = ADSRStage::Attack;
                                voice.envelope = 0;

                                let start_addr_ptr =
                                    (self.sdsp_regs.sample_directory_page as u16) << 8;
                                let start_addr_ptr =
                                    start_addr_ptr + ((voice.sample_source as u16) << 2);
                                ...
                                if H::IS_DEBUGGING_HARNESS && H::TRACK_VOICES {
                                    self.harness.on_voice_key_on(voice, voice_idx);
                                }
                            }
                        }
            ...
            0xD => {
                match nibble_hi {
                    0 => {
                        self.sdsp_regs.echo_feedback = value;
                    }
                    1 => {
                        self.sdsp_regs.unused = value;
                    }
                    2 => {
                    ...
                    5 => {
                        self.sdsp_regs.sample_directory_page = value;
                    }
                    6 => {
                        self.sdsp_regs.echo_page = value;
                    }
                    7 => {
                        self.sdsp_regs.echo_delay_time = value;
                    }
            ...
            0xF => {
                self.sdsp_regs.fir_regs[(nibble_hi & 0x7) as usize] = value as i8;
            }
