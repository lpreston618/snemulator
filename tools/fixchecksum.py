...
def pad_rom(rom_bytes: list[int]) -> list[int]:
    n = len(rom_bytes)
    nOnes = count_ones(n)
    ...
    if nOnes == 2:
        ...
        padded_rom = list(rom_bytes[:larger_size])
        
        for _ in range(repeat_count):
            padded_rom.extend(list(rom_bytes[larger_size:]))
    ...
    if nOnes == 3:
        ...
        padded_rom = list(rom_bytes[:larger_size])
        smaller_part = list(rom_bytes[larger_size:])
...
"""
Returns the updated rom and a bool. The bool is true if the rom was changed
(i.e. the checksum and/or complement was incorrect, false otherwise)
"""
def fix_checksum(padded_rom: list[int], header_pos: int) -> tuple[list[int], bool]:
    rom_mirror = len(padded_rom) - 1
    
    old_checksum_low  = padded_rom[(header_pos + CHECKSUM_OFFSET + 0) & rom_mirror]
    old_checksum_high = padded_rom[(header_pos + CHECKSUM_OFFSET + 1) & rom_mirror]
    old_complement_low  = padded_rom[(header_pos + COMPLEMENT_OFFSET + 0) & rom_mirror]
    old_complement_high = padded_rom[(header_pos + COMPLEMENT_OFFSET + 1) & rom_mirror]
    ...
    complement_low = complement & 0xFF
    complement_high = complement >> 8
    ...
    changed = (old_checksum_low != checksum_low) or (old_checksum_high != checksum_high) or \
              (old_complement_low != complement_low) or (old_complement_high != complement_high)

    return (padded_rom, changed)

def main(rom_filepath: str, rom_mapping_mode: str):
    ...
    if len(padded_rom) == 0:
        print("ROM size should be multiple of two, or sum of 2 or 3 multiples of two")
        print(f"i.e. rom size should have one, two, or three 1's in binary, instead rom size is {bin(len(rom_bytes))}")
        return
    ...
    (fixed_rom, rom_changed) = fix_checksum(padded_rom, header_pos)
    
    if not rom_changed:
        print("Checksum and complement already correct, exiting.")
        return
        
    fixed_rom = fixed_rom[:len(rom_bytes)]
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python fixchecksum.py [ROM filepath] [Mapping mode]")
        ...
        main(rom_filepath, rom_mapping_mode)